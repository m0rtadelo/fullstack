...
{
    _id: "getPaginated",
    value: function (collName, query = {}, offset = 0, limit = 100) {
        const cursor = db[collName].find(query)
        return {
            name: collName,
            total: cursor.count(),
            offset,
            limit,
            data: cursor.limit(limit).skip(offset).toArray()
        }
    }
}
)
// db.fullstack.js.getPaginated = function getPaginated(collName, query = {}, offset = 0, limit = 100) {
//     const cursor = db[collName].find(query)
//     return {
//         name: collName,
//         total: cursor.count(),
...
db.fullstack.js.getValueTables = function getValueTables(query = {}, offset = 0, limit = 100) {
    const cursor = db.valueTables.find(query, {name:1})
    return {
        name: 'valueTables',
        total: cursor.count(),
        offset,
...
function getValueTables() {
    return {
        total: db.valueTables.find().count(),
